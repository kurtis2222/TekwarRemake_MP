...
public class WeaponScript : MonoBehaviour {
	...
	void Start()
	{
		...
		for(int i = 0; i < 3; i++)
			ammo[i] = 100;
		weaponobj1.renderer.enabled = false;
		weaponobj2.renderer.enabled = true;
		weaponobj3.renderer.enabled = false;
		currweap = 2;
	...
	void ShootGun(Rigidbody projectile, AudioClip weapsnd, int sndid, int reduce)
	{
		if(ammo[currweap-1] > 0 && rnd(mainplayer.GetComponent<MainScript>().cos+20))
		{
			...
			playerobj.GetComponent<NetAnim>().PlayAnimation(playerobj.networkView.viewID,"shoot");
			playerobj.audio.PlayOneShot(weapsnd);
			playerobj.GetComponent<NetSnd>().RecSnd(playerobj.networkView.viewID,sndid);
	...
	void ChangeWeapon(int weapid)
	{
		if(currweap != weapid)
		{
			handobj.renderer.enabled = false;
			weaponobj1.renderer.enabled = false;
			weaponobj2.renderer.enabled = false;
			weaponobj3.renderer.enabled = false;
			currweap = weapid;
			tmpweap = currweap;
			blockshot = true;
	...
	IEnumerator ShowHand()
	{
		...
		if(currweap == 1)
		{
			handobj.renderer.enabled = true;
			weaponobj1.renderer.enabled = true;
			weaponobj2.renderer.enabled = false;
			weaponobj3.renderer.enabled = false;
			currweap = 1;	
		}
		...
		if(currweap == 3)
		{
			...
			weaponobj1.renderer.enabled = false;
			weaponobj2.renderer.enabled = false;
			weaponobj3.renderer.enabled = true;
			currweap = 3;
		}
		blockshot = false;
	}
