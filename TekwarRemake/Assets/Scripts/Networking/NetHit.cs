...
public class NetHit : MonoBehaviour {
	...
	void Start()
	{
		...
		if(networkView.isMine) StartCoroutine(HideKilledBy());
		else enabled = false;
	}
	...
	public void SendHit (int hp, int rnd_min, int rnd_max, NetworkViewID id, NetworkPlayer kid, bool isnpc) {
		...
		networkView.RPC("SendNetHit", id.owner, hp, rnd_min, rnd_max,
	...
	void SendNetHit(int hp, int rnd_min, int rnd_max, NetworkPlayer kid, string kname, bool isnpc)
	{
		if(networkView.isMine)
		{
			...
			{
				if(isnpc)
					networkView.RPC("SendKilledByNPC",RPCMode.All,gamecontrol.GetComponent<GameMenu>().player_name);
				else
				{
					gamecontrol.GetComponent<NetScore>().ServAddScore(kname);
					networkView.RPC("SendKilledBy",RPCMode.All,gamecontrol.GetComponent<GameMenu>().player_name,kname);
				}
			}
		...
		else networkView.RPC("SendNetHit", networkView.owner, hp, rnd_min, rnd_max, kid, kname, isnpc);
	}
	
	void SendNetHitServ(int hp, int rnd_min, int rnd_max)
	{
		if(gameObject.GetComponent<MainScript>().hp - hp <= 0)
			networkView.RPC("RecieveKilledByNPC",RPCMode.All,gamecontrol.GetComponent<GameMenu>().player_name);
	...
	IEnumerator HideKilledBy()
	{
		...
		if(count > 0) count -= 1;
		if(count == 0 && gui_killinfo.guiText.enabled) gui_killinfo.GetComponent<GUIText>().enabled = false;
		}
	}
}
