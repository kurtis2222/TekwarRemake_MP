...
public class NetScore : MonoBehaviour {
	...
	void ScoreUpdate()
	{
		...
		GameObject.Find("HudPlayers").guiText.text = list;
		networkView.RPC("SendScore",RPCMode.Others,list);
	}
	...
	{
		networkView.RPC("RemPlayer",RPCMode.Server,pname);
	}
	
	public void ServAddScore(string pname)
	{
		if(Network.isClient)
			networkView.RPC("AddScore",RPCMode.Server,pname);
		else
			AddScore(pname);
	}
}
