...
public class MainScript : MonoBehaviour {
	...
	void Start () {
		...
		{	
			...
			if(Network.isClient)
				gamecontrol.GetComponent<SyncStart>().SendSyncCall(networkView.viewID);
			...
			if(File.Exists("config.ini"))
			{
				...
				while(true)
				{
					...
					if(tmp.StartsWith("Track="))
					{
						...
						MainCam.audio.clip = track[currclip];
						break;	
					}
				}
				sr.Close();
    ...
    void FixedUpdate()
    {
		...
		if(!allowshoot)
		{
			if(Input.GetButton("Run/Sprint") &&
				...
				Skin.GetComponent<NetAnim>().PlayAnimation(Skin.networkView.viewID,"run");
			else if(Input.GetButton("Horizontal") || Input.GetButton("Vertical"))
				Skin.GetComponent<NetAnim>().PlayAnimation(Skin.networkView.viewID,"walk");
			else
				Skin.GetComponent<NetAnim>().PlayAnimation(Skin.networkView.viewID,"idle");
		}
	}
